...
class StudentAgent(BaseAgent):
    def __init__(self, port: int, **kwargs):
        super().__init__("Student", port, seed=None, **kwargs)
        self.extra_args = [
            "--auto-accept-invites",
...
class EnrollmentAgent(BaseAgent):
    def __init__(self, port: int, **kwargs):
        super().__init__("Enrollment", port, seed="random", **kwargs)
        self.extra_args = [
            "--auto-accept-invites",
            "--auto-accept-requests",
            "--monitor-ping",
...
async def main(
    ...
    student = None
    enrollment = None
    student_mediator_agent = None
    enrollment_mediator_agent = None
    ...
    try:
        ...
        await student.listen_webhooks(start_port + 2)
        ...
        await enrollment.listen_webhooks(start_port + 5)
        await enrollment.register_did()

        with log_timer("Startup duration:"):
            await student.start_process()
            await enrollment.start_process()

            if mediation:
                student_mediator_agent = await start_mediator_agent(
                    start_port + 8, genesis, multi_ledger_config_path
                )
                if not student_mediator_agent:
                    raise Exception("Mediator agent returns None :-(")
                enrollment_mediator_agent = await start_mediator_agent(
                    start_port + 11, genesis, multi_ledger_config_path
                )
                if not enrollment_mediator_agent:
                    raise Exception("Mediator agent returns None :-(")
            else:
                student_mediator_agent = None
                enrollment_mediator_agent = None
        ...
        with log_timer("Connect duration:"):
            if multitenant:
                ...
                await student.register_or_switch_wallet(
                    "Student.initial",
                    webhook_port=None,
                    mediator_agent=student_mediator_agent,
                )
                await enrollment.register_or_switch_wallet(
                    "Enrollment.initial",
                    public_did=True,
                    webhook_port=None,
                    mediator_agent=enrollment_mediator_agent,
                )
            elif mediation:
                # we need to pre-connect the agent(s) to their mediator (use the same
                # mediator for both)
                if not await connect_wallet_to_mediator(student, student_mediator_agent):
                    log_msg("Mediation setup FAILED :-(")
                    raise Exception("Mediation setup FAILED :-(")
                if not await connect_wallet_to_mediator(enrollment, enrollment_mediator_agent):
                    log_msg("Mediation setup FAILED :-(")
                    raise Exception("Mediation setup FAILED :-(")

            invite = await enrollment.get_invite(use_did_exchange)
            await student.receive_invite(invite["invitation"])
            await asyncio.wait_for(enrollment.detect_connection(), 30)

        if action != "ping":
            with log_timer("Publish duration:"):
                await enrollment.publish_defs(revocation)
            # cache the credential definition
            await student.fetch_credential_definition(enrollment.credential_definition_id)

        if show_timing:
            await student.reset_timing()
            await enrollment.reset_timing()
            if mediation:
                await student_mediator_agent.reset_timing()
                await enrollment_mediator_agent.reset_timing()

        semaphore = asyncio.Semaphore(threads)

        def done_propose(fut: asyncio.Task):
            semaphore.release()
            student.check_task_exception(fut)

        def done_send(fut: asyncio.Task):
            semaphore.release()
            enrollment.check_task_exception(fut)
        ...
        async def propose_credential(index: int):
            ...
            asyncio.ensure_future(
                student.propose_credential(
                    attributes, enrollment.credential_definition_id, comment, not revocation
                )
        ...
        async def send_credential(index: int):
            ...
            attributes = test_cred(index)
            asyncio.ensure_future(
                enrollment.send_credential(attributes, comment, not revocation)
        ...
        async def send_ping(index: int):
            await semaphore.acquire()
            asyncio.ensure_future(enrollment.send_ping(str(index))).add_done_callback(
                done_send
            )
        ...
        if action == "ping":
            recv_timer = enrollment.log_timer(f"Completed {issue_count} ping exchanges in")
            batch_timer = enrollment.log_timer(f"Started {batch_size} ping exchanges in")
        else:
            recv_timer = enrollment.log_timer(
                f"Completed {issue_count} credential exchanges in"
            )
            batch_timer = enrollment.log_timer(
        ...
        with progress() as pb:
            ...
            try:
                ...
                issue_task = asyncio.ensure_future(
                    check_received(enrollment, issue_count, issue_pg)
                )
                issue_task.add_done_callback(enrollment.check_task_exception)
                receive_task = asyncio.ensure_future(
                    check_received(student, issue_count, receive_pg)
                )
                receive_task.add_done_callback(student.check_task_exception)
                with enrollment.log_timer(completed):
        ...
        enrollment.log(f"Average time per {item_long}: {avg:.2f}s ({1/avg:.2f}/s)")

        if student.postgres:
            await student.collect_postgres_stats(f"{issue_count} {item_short}s")
            for line in student.format_postgres_stats():
                student.log(line)
        if enrollment.postgres:
            await enrollment.collect_postgres_stats(f"{issue_count} {item_short}s")
            for line in enrollment.format_postgres_stats():
                enrollment.log(line)

        if revocation and enrollment.revocations:
            (rev_reg_id, cred_rev_id) = next(iter(enrollment.revocations))
            print(
        ...
        if show_timing:
            timing = await student.fetch_timing()
            if timing:
                for line in student.format_timing(timing):
                    student.log(line)

            timing = await enrollment.fetch_timing()
            if timing:
                for line in enrollment.format_timing(timing):
                    enrollment.log(line)
            if mediation:
                timing = await student_mediator_agent.fetch_timing()
                if timing:
                    for line in student_mediator_agent.format_timing(timing):
                        student_mediator_agent.log(line)
                timing = await enrollment_mediator_agent.fetch_timing()
                if timing:
                    for line in enrollment_mediator_agent.format_timing(timing):
                        enrollment_mediator_agent.log(line)

    finally:
        terminated = True
        try:
            if student:
                await student.terminate()
        except Exception:
        ...
        try:
            ...
            terminated = False
        try:
            if student_mediator_agent:
                await student_mediator_agent.terminate()
            if enrollment_mediator_agent:
                await enrollment_mediator_agent.terminate()
        except Exception:
            LOGGER.exception("Error terminating agent:")
            terminated = False
